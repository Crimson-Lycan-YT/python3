...
#GUI
setup_window = Tk()
setup_window.title("Virtual Pet")
setup_window.geometry("300x200")

lbl_welcome = Label(setup_window, text = "Welcome to the main area!\nWhat is your choice?")
lbl_welcome.grid(column = 1, row = 0)

if(debug_mode == True):
    lbl_welcome.configure(text = "Debug mode enabled.\nWhat is your choice?")

def newClicked():
    lbl_welcome.configure(text = "newButton was clicked")
    nostalSys.initNewGamePlayer(player_base)
...
def loadClicked():
    lbl_welcome.configure(text = "loadButton was clicked")
    global loadflag
    loadflag = True
    setup_window.destroy()
    
new_btn = Button(setup_window, text = "New Game", command = newClicked)
new_btn.grid(column = 0, row = 1)

load_btn = Button(setup_window, text = "Load Game", command = loadClicked)
load_btn.grid(column = 2, row = 1)

lbl_instructions = Label(setup_window, text = "Press \"New Game\" to start \na new game.\nPress \"Load Game\" to load \nfrom existing save data.")
lbl_instructions.grid(column = 1, row = 3)
